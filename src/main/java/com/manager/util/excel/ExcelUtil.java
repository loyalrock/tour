...
public class ExcelUtil {
     ...
     * @return
     */
    public static ExcelWriterFactroy writeExcelWithSheets(HttpServletResponse response, List<String> head,
                                                          List<List<String>> list, String fileName, String sheetName) {
        ExcelWriterFactroy writer = new ExcelWriterFactroy(getOutputStream(fileName, response), ExcelTypeEnum.XLSX);
        Sheet sheet = new Sheet(1, 0);
        sheet.setSheetName(sheetName);
        List<List<String>> headList = new ArrayList<>();
        head.forEach(n -> headList.add(Collections.singletonList(n)));
